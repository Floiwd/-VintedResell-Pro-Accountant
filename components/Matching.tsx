...
const Matching: React.FC<Props> = ({ state, onUpdateInventory }) => {
  ...
  }, [state.inventory, searchTerm, showAllItems]);

  // Smart Matching Logic
  const handleAutoMatch = () => {
    if (!selectedTransfer) return;

    const desc = selectedTransfer.description.toUpperCase();
    ...
    const sizeMatch = desc.match(/\b([W|L]\d{2}|XXS|XS|S|M|L|XL|XXL|XXXL|[0-9]{2,3})\b/i);
    ...
    state.inventory.forEach(item => {
      ...
      if (item.displayId && desc.includes(item.displayId.toUpperCase())) isMatch = true;

      // Match by Name keywords + Size
      if (sizeToSearch && item.size === sizeToSearch && desc.includes(item.brand.toUpperCase())) isMatch = true;
  ...
  // Reset suggestions when changing transfer
  useEffect(() => {
    setSelectedItemIds([]);
  }, [selectedTransferId]);
  ...
  const handleApplyMatching = () => {
    if (!selectedTransfer || selectedItemIds.length === 0) return;

    const pricePerItem = selectedTransfer.amount / selectedItemIds.length;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-bold uppercase text-[10px] tracking-widest mt-2 flex items-center gap-2">
            <Calculator className="w-4 h-4 text-indigo-500" /> Reliez vos flux bancaires à vos articles pour calculer les marges
          </p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        {/* Step 1: Select Purchase Record */}
        ...
        <div className="bg-white dark:bg-[#0F172A] p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col">
              ...
              displayItems.map(item => (
                ...
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="font-black text-slate-900 dark:text-white text-xs truncate">{item.name}</div>
          ...
          <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <div className="text-2xl font-black text-slate-900 dark:text-white">
                  {selectedTransfer && selectedItemIds.length > 0 
                    ? (selectedTransfer.amount / selectedItemIds.length).toFixed(2) 
                    : '0.00'}€
                </div>
              </div>
              <div className="text-right">
                <div className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Total de l'achat</div>
                <div className="text-xl font-black text-indigo-600">
                  {selectedTransfer ? selectedTransfer.amount.toFixed(2) : '0.00'}€
                </div>
              </div>
            </div>
            ...
            <button 
              ...
              disabled={!selectedTransfer || selectedItemIds.length === 0}
              className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-indigo-200 dark:shadow-none transition-all flex items-center justify-center gap-3"
            >
