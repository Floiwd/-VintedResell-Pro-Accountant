...
const Finances: React.FC<Props> = ({ 
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 <div className="bg-indigo-600 rounded-[32px] p-8 text-white relative overflow-hidden group">
                     ...
                     <h3 className="text-3xl font-black mb-1">{analytics.brandChartData[0]?.name || 'N/A'}</h3>
                     <p className="text-xs font-bold opacity-80">+{analytics.brandChartData[0]?.profit.toFixed(0)}€ {t.inventory.item_card.profit.toLowerCase()}</p>
                 </div>
                 <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[32px] p-8 relative overflow-hidden">
                     <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">{t.finances.analytics.best_day}</p>
                     <h3 className="text-3xl font-black text-slate-900 dark:text-white mb-1 flex items-center gap-3">
                         <CalendarDays className="w-8 h-8 text-emerald-500" /> {analytics.bestDay}
                     </h3>
                     <p className="text-xs font-bold text-slate-500">Pic de ventes hebdomadaire</p>
                 </div>
                 <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[32px] p-8 relative overflow-hidden">
                     <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">{t.finances.analytics.total_margin}</p>
                     <h3 className="text-3xl font-black text-emerald-500 mb-1">+{netProfitFromSales.toFixed(0)}€</h3>
                     <p className="text-xs font-bold text-slate-500">Sur {soldItems.length} articles vendus</p>
                 </div>
             </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 ...
                 <div className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm">
                     <h3 className="text-sm font-black uppercase text-slate-900 dark:text-white mb-8 flex items-center gap-2">
  ...
  const renderCashflow = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className={`p-8 rounded-[32px] text-white shadow-2xl relative overflow-hidden group transition-colors duration-500 ${isBelowThreshold ? 'bg-rose-900 shadow-rose-200 dark:shadow-none' : 'bg-slate-900 dark:bg-slate-800 dark:border dark:border-slate-700'}`}>
          <div className="relative z-10">
            ...
            <div className="flex flex-col gap-2 mb-8">
              <h2 className={`text-5xl font-black tracking-tighter ${currentCash >= 0 ? 'text-white' : 'text-rose-500'}`}>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col transition-colors">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                <Users className="w-6 h-6 text-indigo-600 dark:text-indigo-400" /> {t.finances.team.title}
              </h3>
              <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">{t.finances.team.subtitle}</p>
            </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
        <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/30 flex justify-between items-center">
            <div>
               ...
               <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">{t.finances.recurring.subtitle}</p>
            </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
        <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/30">
          <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">{t.finances.cashflow.history}</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-slate-800 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
              <tr>
                <th className="px-8 py-4">{t.common.date} & {t.common.description}</th>
                <th className="px-8 py-4">{t.common.type} / Associé</th>
                <th className="px-8 py-4 text-right">{t.common.amount}</th>
              </tr>
            </thead>
              ...
              {state.transfers.sort((a,b) => b.date.localeCompare(a.date)).map(t => (
                <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-8 py-5">
                    <div className="text-xs text-slate-400 dark:text-slate-500 font-black mb-1">{t.date}</div>
                  ...
                  <td className="px-8 py-5 text-right">
                    <div className="flex items-center justify-end gap-4">
                      <span className={`text-lg font-black ${t.type === 'WITHDRAWAL' ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                        {t.type === 'WITHDRAWAL' ? '-' : '+'}{t.amount.toFixed(2)}€
                      </span>
                      <button onClick={() => setPendingDeleteTransfer(t.id)} className="p-2 text-slate-300 dark:text-slate-600 hover:text-rose-600 dark:hover:text-rose-400 opacity-0 group-hover:opacity-100 transition-all">
  ...
  return (
    <div className={`space-y-6 transition-colors duration-500 p-4 -m-4 rounded-[40px] ${isBelowThreshold && view === 'cashflow' ? 'bg-rose-50/70 dark:bg-rose-900/20 border-2 border-rose-100 dark:border-rose-900' : ''}`}>
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-8">
          <div>
              <h2 className="text-4xl font-black tracking-tighter text-slate-900 dark:text-white uppercase italic">{t.finances.title}</h2>
              <p className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] mt-2">{t.finances.subtitle}</p>
          </div>
          <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800 rounded-2xl">
              <button 
                onClick={() => setView('cashflow')}
                className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${view === 'cashflow' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
              >
                  <Wallet className="w-4 h-4" /> {t.finances.tab_cashflow}
              </button>
              <button 
                onClick={() => setView('analytics')}
                className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${view === 'analytics' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
              >
      ...
      {showTransferForm && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[32px] shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200 text-slate-900 dark:text-white">
            <div className="px-8 py-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center">
              <h2 className="text-2xl font-black uppercase tracking-tight">FLUX DE TRÉSORERIE</h2>
              <button onClick={() => setShowTransferForm(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-900 dark:text-white"><X className="w-6 h-6" /></button>
            </div>
            <form onSubmit={handleTransferSubmit} className="p-8 space-y-6">
              <div>
                <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">{t.common.amount} (€)</label>
                <input name="amount" type="number" step="0.01" required className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-2xl outline-none focus:border-indigo-600 text-slate-900 dark:text-white" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">{t.common.type}</label>
                  <select name="type" className="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold outline-none text-slate-900 dark:text-white">
                ...
                <div>
                  <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">{t.common.date}</label>
                  <input name="date" type="date" defaultValue={new Date().toISOString().split('T')[0]} required className="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-slate-900 dark:text-white" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">Attribution</label>
                <select name="memberId" className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold outline-none text-slate-900 dark:text-white">
              ...
              <div>
                <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">{t.common.description}</label>
                <input name="description" required className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold outline-none text-slate-900 dark:text-white" placeholder="Libellé" />
              </div>
              <div className="flex gap-4">
                <button type="button" onClick={() => setShowTransferForm(false)} className="flex-1 px-6 py-4 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest text-xs hover:bg-slate-50 dark:hover:bg-slate-800">{t.common.cancel}</button>
                <button type="submit" className="flex-1 px-6 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl uppercase tracking-widest text-xs hover:bg-indigo-700">{t.common.validate}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showRecurringForm && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[32px] shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200 text-slate-900 dark:text-white">
            <div className="px-8 py-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center">
              <h2 className="text-2xl font-black uppercase tracking-tight">{t.finances.recurring.form_title}</h2>
              <button onClick={() => setShowRecurringForm(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-900 dark:text-white"><X className="w-6 h-6" /></button>
            </div>
            <form onSubmit={handleRecurringSubmit} className="p-8 space-y-6">
              <div>
                <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">{t.finances.recurring.name_label}</label>
                <input name="name" required placeholder="Ex: Netflix, Loyer, Logiciel..." className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold outline-none focus:border-indigo-600 text-slate-900 dark:text-white" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">{t.finances.recurring.amount_label} (€)</label>
                    <input name="amount" type="number" step="0.01" required className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-xl outline-none focus:border-indigo-600 text-slate-900 dark:text-white" />
                  </div>
                  <div>
                    <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">{t.finances.recurring.freq_label}</label>
                    <select name="frequency" className="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold outline-none text-slate-900 dark:text-white cursor-pointer">
              ...
              <div>
                <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">{t.finances.recurring.due_date}</label>
                <input name="nextDueDate" type="date" defaultValue={new Date().toISOString().split('T')[0]} required className="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-slate-900 dark:text-white" />
              </div>
              
              <div className="flex gap-4 pt-2">
                <button type="button" onClick={() => setShowRecurringForm(false)} className="flex-1 px-6 py-4 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest text-xs hover:bg-slate-50 dark:hover:bg-slate-800">{t.common.cancel}</button>
                <button type="submit" className="flex-1 px-6 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl uppercase tracking-widest text-xs hover:bg-indigo-700">{t.common.add}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showMemberForm && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-sm rounded-[32px] shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200 text-slate-900 dark:text-white">
            <div className="px-8 py-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center">
              <h2 className="text-2xl font-black uppercase tracking-tighter">{editingMember ? t.common.edit : t.common.add} {t.finances.team.add_member}</h2>
              <button onClick={() => setShowMemberForm(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-900 dark:text-white"><X className="w-6 h-6" /></button>
            </div>
            <form onSubmit={handleMemberSubmit} className="p-8 space-y-6">
              <div>
                <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">Nom de l'associé</label>
                <input name="name" defaultValue={editingMember?.name} required className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold outline-none focus:border-indigo-600 text-slate-900 dark:text-white" />
              </div>
              <div>
                <label className="block text-xs font-black text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-widest">Parts (%)</label>
                <input name="sharePercentage" type="number" min="0" max="100" defaultValue={editingMember?.sharePercentage || 50} required className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-xl outline-none focus:border-indigo-600 text-slate-900 dark:text-white" />
              </div>
              <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setShowMemberForm(false)} className="flex-1 px-6 py-4 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest text-xs hover:bg-slate-50 dark:hover:bg-slate-800">{t.common.cancel}</button>
                <button type="submit" className="flex-1 px-6 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl uppercase tracking-widest text-xs hover:bg-indigo-700">{t.common.confirm}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showInviteForm && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[32px] shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200 text-slate-900 dark:text-white">
            <div className="px-8 py-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center">
              <h2 className="text-2xl font-black uppercase tracking-tight">{t.finances.team.invite}</h2>
              <button onClick={() => setShowInviteForm(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-900 dark:text-white"><X className="w-6 h-6" /></button>
            </div>
            <form onSubmit={handleInviteSubmit} className="p-8 space-y-6">
              <div>
                <label className="block text-xs font-black mb-2 uppercase tracking-widest text-slate-500 dark:text-slate-400">E-mail de l'associé</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold outline-none focus:border-emerald-500 text-slate-900 dark:text-white" 
                  />
                </div>
              </div>
              <div className="flex gap-4">
                <button type="button" onClick={() => setShowInviteForm(false)} className="flex-1 px-6 py-4 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest text-xs hover:bg-slate-50 dark:hover:bg-slate-800">{t.common.cancel}</button>
                <button type="submit" className="flex-1 px-6 py-4 bg-emerald-600 text-white rounded-2xl font-black shadow-xl uppercase tracking-widest text-xs hover:bg-emerald-700">{t.finances.team.invite}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showThresholdForm && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-sm rounded-[32px] shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200 text-slate-900 dark:text-white">
            <div className="px-8 py-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center">
              <h2 className="text-2xl font-black uppercase tracking-tight">SEUIL DE SÉCURITÉ</h2>
              <button onClick={() => setShowThresholdForm(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-900 dark:text-white"><X className="w-6 h-6" /></button>
            </div>
            <form onSubmit={handleThresholdSubmit} className="p-8 space-y-6">
              <div>
                <label className="block text-xs font-black mb-2 uppercase tracking-widest text-slate-500 dark:text-slate-400">Montant d'alerte (€)</label>
                <input name="threshold" type="number" step="0.01" defaultValue={state.cashThreshold} required className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-2xl outline-none focus:border-indigo-600 text-slate-900 dark:text-white" />
              </div>
              <div className="flex gap-4">
                <button type="button" onClick={() => setShowThresholdForm(false)} className="flex-1 px-6 py-4 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest text-xs hover:bg-slate-50 dark:hover:bg-slate-800">{t.common.cancel}</button>
                <button type="submit" className="flex-1 px-6 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl uppercase tracking-widest text-xs hover:bg-indigo-700">{t.common.validate}</button>
              </div>
            </form>
          </div>
